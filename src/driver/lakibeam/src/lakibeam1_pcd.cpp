...
int main(int argc, char **argv)
{
    ...
    if(bind(sockfd, (struct sockaddr*)&ser_addr, sizeof(ser_addr)) < 0) return -1;
    ...
    while(ros::ok()) {
        if(scan_vec_ready == 0) {
        	...
        	cloud2_.header.frame_id = frame_id.c_str();
                ...
                iter_time[0] = (scan_vec[i].timestamp - scan_vec[0].timestamp) * 0.000001;
                    ++iter_x;
                    ++iter_time;
                }
