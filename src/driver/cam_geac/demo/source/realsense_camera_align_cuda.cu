#include "../inc/librealsense/rscuda_utils.cuh"
#include <device_launch_parameters.h> 
#include <cuda.h>
#include "../inc/librealsense/cuda-align.cuh"
...
std::shared_ptr<rsutils::number::int2>           _d_pixel_map;
...
__device__ void kernel_transfer_pixels(rsutils::number::int2* mapped_pixels, const rs2_intrinsics* depth_intrin,
...
__global__ void kernel_map_depth_to_other(rsutils::number::int2* mapped_pixels, const uint16_t* depth_in, const rs2_intrinsics* depth_intrin, const rs2_intrinsics* other_intrin,
...
// void kernel_depth_to_other(uint16_t* aligned_out, const uint16_t* depth_in, const rsutils::number::int2* mapped_pixels, const rs2_intrinsics* depth_intrin, const rs2_intrinsics* other_intrin)
// {
//     int depth_x = blockIdx.x * blockDim.x + threadIdx.x;
//     int depth_y = blockIdx.y * blockDim.y + threadIdx.y;
...
//     rsutils::number::int2 p0 = mapped_pixels[depth_pixel_index];
//     rsutils::number::int2 p1 = mapped_pixels[depth_size + depth_pixel_index];
...
__global__ void kernel_depth_to_other(uint16_t* aligned_out, const uint16_t* depth_in, const rsutils::number::int2* mapped_pixels, const rs2_intrinsics* depth_intrin, const rs2_intrinsics* other_intrin)
{
    int depth_x = blockIdx.x * blockDim.x + threadIdx.x;
    int depth_y = blockIdx.y * blockDim.y + threadIdx.y;
    ...
    if (depth_pixel_index >= depth_size)
        return;
    ...
    // 获取深度值
    uint16_t depth_val = depth_in[depth_pixel_index];
    ...
    for (int y = p0.y; y <= p1.y; ++y)
    {
        ...
        {
            auto other_pixel_index = y * other_intrin->width + x;
            if (other_pixel_index >= other_intrin->width * other_intrin->height)
                continue;
            
            uint16_t current = aligned_out[other_pixel_index];
            
            // 简单的非原子比较和赋值
            // 对于深度图对齐，这种竞争通常不频繁，结果可接受
            if (current == 0 || depth_val < current) {
                aligned_out[other_pixel_index] = depth_val;
            }
        }
    }
}
    ...
    const rs2_intrinsics& h_other_intrin)
{
    ...
    if (!_d_depth_intrinsics) _d_depth_intrinsics = rscuda::make_device_copy(h_depth_intrin);
    if (!_d_other_intrinsics) _d_other_intrinsics = rscuda::make_device_copy(h_other_intrin);
    if (!_d_depth_other_extrinsics) _d_depth_other_extrinsics = rscuda::make_device_copy(h_depth_to_other);

    if (!_d_depth_in) _d_depth_in = rscuda::alloc_dev<uint16_t>(depth_pixel_count);
    cudaMemcpy(_d_depth_in.get(), h_depth_in, depth_byte_size, cudaMemcpyHostToDevice);

    if (!_d_aligned_out) _d_aligned_out = rscuda::alloc_dev<unsigned char>(aligned_byte_size);
    cudaMemset(_d_aligned_out.get(), 0xff, aligned_byte_size);

    if (!_d_pixel_map) _d_pixel_map = rscuda::alloc_dev<rsutils::number::int2>(depth_pixel_count * 2);
