...
max96712_status_check(){
    ...
    while true; do
        ...
        echo -e "\n串化器是否输出数据"
        i2ctransfer -f -y $max96712_bus w2@$serial_addr 0x08 0xad r4
        ...
        echo -e "\nmax96712是否输出数据,跳变即为输出"
        echo -n "csi   tx0/1: "
        i2ctransfer -f -y $max96712_bus w2@$max96712_addr 0x08 0xD0 r1
        echo -n "csi   tx2/3: "
        i2ctransfer -f -y $max96712_bus w2@$max96712_addr 0x08 0xD1 r1
        echo -n "mipi phy0/1: "
        i2ctransfer -f -y $max96712_bus w2@$max96712_addr 0x08 0xD2 r1
        echo -n "mipi phy2/3: "
        i2ctransfer -f -y $max96712_bus w2@$max96712_addr 0x08 0xD3 r1


        echo -e "\nmax96712是否开流0x40B: bit1"
        i2ctransfer -f -y $max96712_bus w2@$max96712_addr 0x04 0x0B r1
        echo -e "\nmax96712是否开流0x8A0: bit7"
        i2ctransfer -f -y $max96712_bus w2@$max96712_addr 0x08 0xA0 r1
        sleep 1
    done
}
