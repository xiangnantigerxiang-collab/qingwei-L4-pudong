...
void LatController::setParameters(double wheelbase, double max_steer, double max_steer_rate)
{
    wheelbase_ = wheelbase;
    max_steer_ = max_steer / 180 * M_PI;
    max_steer_rate_ = max_steer_rate / 180 * M_PI;
}
double LatController::calculate(const std::vector<Pose2d> &trajectory, const Pose2d &ego_pose,
                                const double &ego_speed, const double &ego_steer_angle)
{
    ...
    double k = 1 / (1 + 1.5 * curvature);
    printf("=============================LatController calculate=============================\n");
    // printf("wheelbase:%f, max_steer:%f, max_steer_rate:%f\n", wheelbase_, max_steer_, max_steer_rate_ * 180 / M_PI);
    ...
    double fusion_angle = feedforwad_steer_angle;
    printf("融合后转向角 %.1f 度\n", fusion_angle / M_PI * 180);

    double filter_steer_angle = this->lowPassFilter(fusion_angle, ego_steer_angle, 0.8);
...
double LatController::calcSteer(const std::vector<Pose2d> &trajectory, const Pose2d &ego_pose,
                                const double &ego_speed)
{
    ...
    double phi_t = math_utils::minusPi2Pi(foot_pose.heading - front_pose.heading);
    // printf("foot heading: %f, ego heading:%f\n", foot_pose.heading, front_pose.heading);
    // printf("横向偏差:%f, 航向偏差:%f\n", e_t, phi_t / M_PI * 180);
    //  4. 套用stanley公式，求解转向角
    double k = 0.6;
    double phi_weight = 0.2;
    ...
    printf("LatController: 横向偏差:%.2f, 航向偏差:%.2f,偏角1:%.1f, 偏角2:%.1f,转向角: %.1f 度\n",
           e_t, phi_t / M_PI * 180, angle1 / M_PI * 180, angle2 / M_PI * 180, steer_angle / M_PI * 180);
    return steer_angle;
}

double LatController::feedforward(const std::vector<Pose2d> &trajectory, const Pose2d &ego_pose, const double &ego_speed)
{
    ...
    // printf("ff_point.x: %f, ff_point.y: %f\n", ff_point.x, ff_point.y);
    // printf("ego_pose.x: %f, ego_pose.y: %f, ego_pose.heading: %f, ego_pose.v: %f\n", ego_pose.x, ego_pose.y, ego_pose.heading, ego_pose.v);
    // printf("p_to_base.x: %f, p_to_base.y: %f, p_to_base.heading: %f\n", p_to_base.x, p_to_base.y, p_to_base.heading);
    // printf("ego heading:%f, ffheading:%f, ptobase heading:%f\n", ego_pose.heading, ff_point.heading, p_to_base.heading);
    double turn_radius = (std::pow(p_to_base.x, 2) + std::pow(p_to_base.y, 2)) / (2 * p_to_base.y);
    // printf("turn_radius:%f\n", turn_radius);
    // 3. 计算转向角
    double steer_angle = atan(wheelbase_ / turn_radius);
    printf("puresuit 前视距离:%f, 转弯半径:%f, 转向角: %f 度\n", forward_distance, turn_radius, steer_angle / M_PI * 180);
    // double delta_ff = atan(ff_point.curvature * wheelbase_);
    return steer_angle;
}

double LatController::steerLimit(double desired_steer_angle, double current_steer_angle)
{
    ...
    // printf("-----------------------\n");
    //  printf("dangle:%f,cangle:%f,diff:%f\n", desired_steer_angle, current_steer_angle, steer_diff);
    // printf("-----------------------\n");
    return steer_angle;
}

double LatController::lowPassFilter(double new_val, double old_val, double alpha)
{
...
double LatController::calcCurature(const std::vector<Pose2d> &trajectory, const Pose2d &ego_pose)
{
    double curature = 0.0;
