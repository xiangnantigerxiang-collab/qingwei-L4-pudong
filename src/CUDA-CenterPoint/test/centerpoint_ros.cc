...
namespace kParams {
  ...
  static const std::vector<AABB> kEgoZones = {
    ...
    { -3.9f,  2.3f, -1.0f, 1.0f, -2.0f, 2.5f },//ziche
    // optional extra zone, e.g. roof sensor blind-spot – uncomment to use:
    { -3.9f,  1.5f, -2.0f, 2.0f,  0.5f, 3.0f },//shensuogan
    { 0.5f,  1.5f, -2.0f, 2.0f,  -10.7f, 10.2f },//houshijing
    { 2.0f,  5.0f, 1.0f, 5.0f,  -50.0f, 20.5f },//
    { 2.0f,  5.0f, -5.0f, -1.0f,  -50.0f, 20.5f },//
    { 1.5f,  2.2f, 0.8f, 1.3f,  -50.0f, 20.5f },//
...

{
  ...
  if (!lidar_ok) {
    printf("[CenterPoint]  Lidar健康检查失败: %s, 关闭publisher\n", lidar_issue.c_str());
    ShutdownPublisher();
    return;
  }

  // ── Lidar 恢复后，重新创建 publisher ────────────────────────
  if (!publisher_valid_) {
    RestorePublisher();
  }
  ...
  if (total_pts < kParams::kMinTotalPoints) {
    printf("[CenterPoint] 总点数不足: %d < %d, 关闭publisher\n", 
  ...
  //PubDetectedMarker(centerpoint_ptr_->nms_pred_);
...

{
  ...
  for (const auto& box : boxes) {
    ...
    float thresh = kParams::GetThresholdForBox(box.id, box.x, box.y);
    ...
    if (IsEgoPoint(box.x, box.y, box.z)) continue;
