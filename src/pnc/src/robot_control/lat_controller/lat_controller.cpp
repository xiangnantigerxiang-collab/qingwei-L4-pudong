...
double LatController::calculate(const std::vector<Pose2d> &trajectory, const Pose2d &ego_pose,
                                double ego_speed, double ego_steer_angle, uint Gear)
{
    ...
    printf("自车转向角: %.1f, 自车车速:%.1f, 自车曲率:%.1f, k系数:%.1f\n", ego_steer_angle / M_PI * 180, 
        ego_speed, curvature, k);
    printf("----------------------------------------------------------------------------\n");
    ...
    k= 0.3;
    double fusion_angle = (1 - k) * stanley_steer_angle + k * feedforwad_steer_angle;
