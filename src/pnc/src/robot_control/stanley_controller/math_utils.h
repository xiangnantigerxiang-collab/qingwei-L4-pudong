// 横向控制器几何工具（math_utils 命名空间）
// 垂足投影、最近点查找、路径稠密化、s/曲率属性计算

#ifndef ROBOT_CONTROL_STANLEY_CONTROLLER_MATH_UTILS_H_
#define ROBOT_CONTROL_STANLEY_CONTROLLER_MATH_UTILS_H_
 ...
 */
template <typename T>
std::vector<T> denseLineString(const std::vector<T>& ls,
  ...
  for (int i = 1; i < ls.size(); ++i) {
    ...
    for (int j = 0; j < count; ++j) {
      ...
      T insert_pt;
      insert_pt.x =
          last_point.x + deta / length * (next_point.x - last_point.x);
      insert_pt.y =
          last_point.y + deta / length * (next_point.y - last_point.y);
...
template <typename T>
std::vector<T> sparseLineString(const std::vector<T>& ls,
                                const double& interval) {
  if (ls.size() <= 2) {
    return ls;
  }
...
double computeCurvature(const T& A, const T& B, const T& C) {
  // 外接圆面积公式: S = abc/4R
  // 计算两个相邻点之间的距离
  double l_AB = std::sqrt((B.x - A.x) * (B.x - A.x) +
                          (B.y - A.y) * (B.y - A.y));
  double l_BC = std::sqrt((C.x - B.x) * (C.x - B.x) +
                          (C.y - B.y) * (C.y - B.y));
  double l_AC = std::sqrt((C.x - A.x) * (C.x - A.x) +
                          (C.y - A.y) * (C.y - A.y));
  ...
  double S = 0.5 * (std::abs((B.x - A.x) * (C.y - A.y) -
                             (C.x - A.x) * (B.y - A.y)));
  if (fabs(S) < 1e-6) {
    return 0;
  } else {
