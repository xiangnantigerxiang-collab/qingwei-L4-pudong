...
#include "robot_task_plan/fence_geometry.h"
...
#include "safety/startup_observation.h"
...
public:
    ...
    void EnableFenceGuard() { mFenceEnabled = true; }
    void SetFenceGuard(const robot::task_fence_guard &guard);
    void PublishFenceFeedback(ros::Publisher &publisher);
    ...
    void SetStartupPerceptionData(const robot::perception &perception_t);
    ...
    // 转向灯防抖与状态保持
    int mTurnLightCandidate = 0;
    int mTurnLightCandidateCount = 0;
    int mActiveTurnLight = 0;
    double mTurnLightActiveTime = -1.0;
    int mActiveTurnLightSteps = 0;
    int mTurnLightOffCount = 0;

private:
    bool FenceTaskRequired() const;
    bool FenceGuardFresh() const;
    void TryLoadFenceTask();
    void FailFence(const std::string &reason);
    bool CheckFencePath(const std::vector<float> &x, const std::vector<float> &y) const;
    bool FenceOutputStop();
    bool mFenceEnabled = false, mFencePending = false, mFenceFailed = false;
    bool mFenceExempt = true;
    uint64_t mFenceTaskKey = 0, mFenceLoadedRevision = 0;
    double mFenceReceived = -1;
    robot::task_fence_guard mFenceGuard;
    task_fence::Geometry mTaskFenceGeometry;
    std::string mFenceFailure;
    // 全局路径构建阶段；实现见 task/global_path.inc。
    void LoadTaskPaths(const robot::task_plan_msg &task_plan_t);
    double GetMapSpeedLimit() const;
    ...
    planning_perception::LaneMotionFilter BuildPerceptionLaneMotionFilter() const;
    ...
    bool IsGantryAheadOnReference() const;
...
private:
    ...
    int go_task_id_ = 0;
    int back_task_id_ = 0;
    bool mGantryStop = false;  // 有效关闭输入；离开参考线触发范围后清除，等待新消息。
    ...
    planning_perception::StartupObservation mStartupObservation;
    double mFinalSafetyLogTime = -1;
    int mFinalSafetyLogSources = -1;
    const char* mStartupLogReason = "";
};
