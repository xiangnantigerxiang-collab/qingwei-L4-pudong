    ...
    const float lidar_x, const float lidar_y, const float vehicle_angle, const float distance, const int pallettype)
{
    ...
    if(pallettype == 0) angle_temp[0] = vehicle_angle;
    else angle_temp[0] = cur_head + 180.0 - atan(lidar_y/lidar_x)*180.0/M_PI;
    ...
    if(pallettype == 0) {
        ...
        hook_pos[0] = lidar_x;
        hook_pos[1] = lidar_y;
    }else board_angle = cur_head + vehicle_angle;
     ...
     const float stop_x,const float stop_y,const float stop_angle)
{
    ...
    for(int i = 1; i < size; i++) {
        ...
        if(detaX > -0.0001 && detaX < 0.0001) {
            if(detaY > 0) angle_temp = 0;
            else angle_temp = 180;
        }else{
            angle_temp = atan((float)detaY / detaX) / M_PI * 180;
            if(detaX > 0) angle_temp = 90 - angle_temp;
            else angle_temp = 270 - angle_temp;
        }
