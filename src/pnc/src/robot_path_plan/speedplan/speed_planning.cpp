...
void Speed_Planning::FreeDrivingInPath(
    ...
    switch(road_direction_) {
        case 1:
            ...
            SpdCurMin = kSpdMin_Forward;
            break;
        case 2:
            SpdMax = kSpdMax_Reverse;
            SpdCurMin = kSpdMin_Reverse;
            break;
    ...
    for(size_t j = (*path).size() - 1; j > 0; --j) {
        ...
        if(fabs((*path).at(j).speed - VelocityInvalid) > 0.1) {  //don't smooth
            ...
            if(dv > kDiffMaxDeceleration) {
                double accel = -0.5 * ((*path).at(0).speed * (*path).at(0).speed - (*path).at(j).speed * (*path).at(j).speed) / (*path).at(j).length;
...
float Speed_Planning::CrossSpeedCalculation(double objDist, float speedBase, float objSpeed) {
    ...
    if(objDist <= PassWidthLower) {
        ...
        crossSpeed = BesideObjCrossSpeedMin + (veh2kPass1 - BesideObjCrossSpeedMin) * (objDist - PassWidthLower) / (PassWidthUpper1 - PassWidthLower) + objSpeed;
    } else if(objDist < PassWidthUpper2) {
        crossSpeed = veh2kPass1 + (veh2kPass2 - veh2kPass1) * (objDist - PassWidthUpper1) / (PassWidthUpper2 - PassWidthUpper1) + objSpeed;
    } else if(objDist < PassWidthUpper3) {
        crossSpeed = veh2kPass2 + (veh2kPass3 - veh2kPass2) * (objDist - PassWidthUpper2) / (PassWidthUpper3 - PassWidthUpper2) + objSpeed;
    } else if(objDist < PassWidthUpper4) {
        crossSpeed = veh2kPass3 + (speedBase - veh2kPass3) * (objDist - PassWidthUpper3) / (PassWidthUpper4 - PassWidthUpper3) + objSpeed;
    } else {
