 ...
 * @Author: your name
 * @Date: 2019-09-18 11:06:48
 * @LastEditTime: 2019-09-20 13:51:46
...
template <typename T, int M, int N, typename D>
    void DenseToCSCMatrix1(const Eigen::Matrix<T, M, N> &dense_matrix,
...
void DenseToCSCMatrix(const Eigen::MatrixXd &dense_matrix,
                      std::vector<T>& data, std::vector<D>& indices,
                      std::vector<D>& indptr) {
    constexpr double epsilon = 1e-6;
    int data_count = 0;
