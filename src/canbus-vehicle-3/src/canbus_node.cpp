...
//load 4 position limit keys from config.cfg; invalid pairs keep defaults.
//hook: 0<=min<max<=255; pallet: min=up, max=down, either numeric order.
//both endpoint pairs must be at least 20 raw units apart.
static int LoadPositionConfig(const char *path)
{
    ...
    while(fgets(line, sizeof(line), fp)) {
        ...
        for(int i = 0; i < 4; i++) {
            ...
            if(*eq != '=') continue;
            if(i >= 2) {
                //反向标定允许 0 作端点，不能把非数字经 atoi 误读为 0。
                char *end = NULL;
                long value = strtol(eq + 1, &end, 10);
                if(end == eq + 1 || value < 0 || value > 255) {
                    seen[i] = 0;
                    continue;
                }
    ...
    if(seen[2] && seen[3] && vals[2] >= 0 && vals[2] <= 255 &&
       vals[3] >= 0 && vals[3] <= 255 && abs(vals[3] - vals[2]) >= 20) {
