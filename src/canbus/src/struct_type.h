...
typedef struct xyz_coor_s{
    xyz_coor_s(float x_axis = 0,float y_axis = 0,float z_axis = 0,float heading = 0,float p2pDistance = 0,\
                float curvature = 0,float velocity = 0,float dist_origin = 0){
            this->x_axis = x_axis;
            this->y_axis = y_axis;
            this->z_axis = z_axis;
...
typedef enum subActionValue{
	...
	HOOKOPERATION = 5,//挂钩
    DECOUPLING = 6,//脱钩
    WAITING = 7,//等待下一步,平台车的信号，接货或者卸货完成
    COUNTERPOINT = 8,//对位,包含坐标和确认信号
	ASKPALLETPOS = 9,
	CALCUBACKPATH = 10,
    UNLOAD_LOWER = 11,
}SUBACTION_E;
...
typedef struct fms_path_s{
	...
	string absolute_speed;
	string relative_speed;
	PATHTYPE path_type;
}FMS_PATH_S;

typedef struct path_info{
	string curPathName;
	vector<XYZ_COOR_S> curPathList;
	string next1PathName;
	vector<XYZ_COOR_S> next1PathList;
	string next2PathName;
	vector<XYZ_COOR_S> next2PathList;
}PATH_INFO_S;
...
struct Point
{
    float x;          // int16_t x;    //relative coordinate system，unit： cm
    float y;          //int16_t y;
    float length;
    float curv;
    float speed;
