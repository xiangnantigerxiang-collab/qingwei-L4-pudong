...
记录器与8083状态页已实现；实车Timer的float.to_sec错误改为传 `rospy.Duration`。
历史46用例/冒烟为本机结果，车端事件窗口、磁盘满降级和相机话题仍需部署核对。

## 注意事项与容易疏忽的点

- 新启动尚未积满90秒时不能凭空补足事件前窗；验收前先让缓冲积累。
- 视频只记录链路状态，未存视频流；TOPIC_CAMERA需按车端实际话题核对。
- 无数据置null/stale，不把默认零值当正常；JSON中的NaN/inf需按现有消毒规则处理。
- 事件关闭/启动时轮转，磁盘上限不代表正在写的事件永远不能超过上限。
- 状态页故障与记录故障分开；SIGKILL后未完成事件不应被写成complete=true。
- 启动用下文工程根命令；测试先进入log_online目录。本轮文档整理未自动接入整车启动脚本。
