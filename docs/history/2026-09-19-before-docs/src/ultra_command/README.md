...
|---|---|---|
| `pudong_air/312_316_01`、`pudong_air/312_cargo_01` | `pudong_air/left1` + `pudong_air/left2` | 障碍物落入任一矩形 → `/ultra/status/safe = 1`；连续三帧均无 → `0` |
| `pudong_air/312_charge_01` | `pudong_air/right` | 障碍物落入 → `1`；连续三帧均无 → `0` |
| `pudong_air/312_316_01_01`、`pudong_air/312_cargo_01_01`（初始化监控） | 同上 left1+left2 | **起步前**（车速<0.5m/s）：区内有障碍 → `1`，无 → `0`；**首次车速>0.5m/s 后**：无论区内是否有障碍恒 `0` |
...
└── src/
    ├── ultra_command_node.cpp     # ROS 薄壳: 订阅 task/perception/can/cloud command
...
5. **启动先写 `safe=1`**：任务状态、感知和矩形尚未确认时保持故障态。
   `/task_plan_msg` 发布器使用 latch，并在 task_plan 启动和云端停车时发布明确的
   空任务；因此晚启动/中途重启能立即恢复当前模式，已知待命则写 0。
6. **矩形加载失败按故障态处理**（写 1）+ 醒目打印；加载成功后仍需新感知帧确认。
   ...
   校验跳过，顶点不足 3 判加载失败。
7. **云端停止指令即时退监控**：task_plan 清任务池后同步发布空任务刷新 latch；
   ultra_command 同时保留 `/cloud/msg/command_msg` 直达清空，兼容旧版并缩短退出
   延迟。暂停/继续不清模式（任务仍在执行）。
...
9. **初始化监控（_01_01 路径，2026-09-11 新增）的起步判据**：
   - 车速源 = `/can_msg.vehicleSpeed`（float32 m/s，canbus 20Hz，全仓标准车速）。
     未收到有效车速时按故障态写 1；最新车速会缓存，CAN 与 task 回调先后顺序不影响
     起步状态。暂不做车速老化——canbus 死则整车已进急停链；
   - 阈值为**严格大于 0.5 m/s**（恰 0.5 不触发）；起步闩锁**单向**，此后降速/
     停车不恢复起步前监测；闩锁按**完整命中路径 + task_id** 标识一次任务激活。
     两个 INIT_LEFT 路径互切、或同路径但 task_id 改变时均重新初始化；相同消息因
     latch 重送不会误复位；
...
4. **消费方契约未验证**：`/ultra/status/safe` 目前全仓零读者（消费方在仓外车载
   环境）。需确认其接受：普通目标任务内障碍/监控异常=1、连续三帧无障碍=0；
   初始化任务起步前按当前帧 1/0、起步后恒 0；非目标任务=0；10Hz rosparam 轮询。
5. **三个 `_01_01` 轨迹文件已存在，但任务 YAML 尚未引用**：三个文件分别与对应
   `_01.csv` 逐字节一致，path_plan 可以正常加载；当前 param/*.yaml 仍全部使用旧
   路径名，因此现役任务不会触发初始化模式。上线前需由任务编排明确替换对应任务块。
...
- 启动：已由根目录 `launch/control.launch` include；`start_l4.sh` 与 HMI 启动 PNC
  时会一并启动。单独调试可用 `roslaunch ultra_command ultra_command.launch`。
- 编后核对：
